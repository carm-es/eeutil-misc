...
public class UsuariosController {
	...
	public @ResponseBody ResultadoBusqueda<UsuarioObject> usuariosListaJson(@RequestParam(value = "app", required = true) String app,
			HttpServletRequest request, Locale locale) {
		ResultadoBusqueda<UsuarioObject> usuarios = new ResultadoBusqueda<UsuarioObject>();
		try {
			String filterString = StringUtils.isNotEmpty(request.getParameter("search[value]")) ? request.getParameter("search[value]") : "";
			int defaultSize = StringUtils.isNotEmpty(request.getParameter("length")) ? Integer.parseInt(request.getParameter("length")) : DEFAULT_SIZE;
			FilterPageRequest filter = new FilterPageRequest(filterString, DEFAULT_PAGE, defaultSize, null);
	...
	public @ResponseBody UnidadResponse usuariosListaFromNif(@RequestParam String user, HttpServletRequest request,
			Locale locale) {
	...
	public @ResponseBody List<ComboItem> autocompleteNumeroProcedimiento(
		...
		// Se obtienen los número de procedimiento
		logger.info("Buscamos los numeros de procedimiento...");

		List<ComboItem> listaReturn = new ArrayList<ComboItem>();
		try {
			List<ProcedimientoObject> numeroProcedimientoList = eeutilUsuarioService.getListaProcedimientos(codigo);
		...
		logger.info("[FIN] Salimos de autocompleteNumeroProcedimiento. Total a mostrar: " + listaReturn.size());
