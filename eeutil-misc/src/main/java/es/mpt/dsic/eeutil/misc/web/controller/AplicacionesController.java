...
public class AplicacionesController {
	...
	protected static final Log logger = LogFactory
			.getLog(AplicacionesController.class);
	...
	public ModelAndView aplicacionesLista(
		...
		try {
			retorno.addObject("data",
					eeutilAplicacionService.getAplicaciones(app));
			...
			option[0] = app;
			retorno.addObject("titulo", messageSource.getMessage(
					"listaAplicaciones.title", option, locale));
	...
	public ModelAndView bajaAplicacion(HttpServletRequest request,
			Locale locale) {
		ModelAndView retorno = null;
		try {
			...
			EeutilAplicacion apli = (EeutilAplicacion) eeutilDao.getObjeto(
					EeutilAplicacion.class, aplicacion.getIdentificador());
			if (apli.isActiva()) {
				aplicacion = eeutilAplicacionService
						.desactivar(app, aplicacion);
			} else {
				aplicacion = eeutilAplicacionService
						.activar(app, aplicacion);
			}
	...
	public ModelAndView eliminarAplicacion(HttpServletRequest request,
			Locale locale) {
		ModelAndView retorno = null;
		String app = request.getParameter("app");
		try {
			...
			AplicacionObject aplicacion = new AplicacionObject();
			aplicacion.setIdentificador(request.getParameter("identificador"));
			eeutilAplicacionService
					.eliminarAplicacion(app, aplicacion, locale);
			
			retorno = aplicacionesLista(app, request, locale);
			
			MessageObject mensaje = new MessageObject(
					WebConstants.MESSAGE_LEVEL_SUCCESS,
					messageSource.getMessage("listaAplicaciones.eliminar.ok",
							null, locale));
			...
			retorno = aplicacionesLista(app, request, locale);
			MessageObject mensaje = new MessageObject(
					WebConstants.MESSAGE_LEVEL_ERROR,
					e.getMessage());
			retorno.addObject("mensajeUsuario", mensaje);
		} catch (CSVStorageException e) {
			logger.error("Error al eliminar aplicacion");
	...
	public ModelAndView altaAplicacion(HttpServletRequest request, Locale locale) {
		...
		try {
			...
			if (aplicacion != null) {
				aplicacion = eeutilAplicacionService.altaAplicacion(app,
						aplicacion);
				mensaje = new MessageObject(
						WebConstants.MESSAGE_LEVEL_SUCCESS,
						messageSource.getMessage("listaAplicaciones.alta.guardar.ok",
								null, locale));
			} else {
				mensaje = new MessageObject(
						WebConstants.MESSAGE_LEVEL_WARNING,
						messageSource.getMessage("error.msg.datosNoValidos",
								null, locale));
			}
			retorno = aplicacionesLista(app, request, locale);
			retorno.addObject("mensajeUsuario", mensaje);
	...
	private AplicacionObject convertToObject(HttpServletRequest request) {
		if (StringUtils.isEmpty(request.getParameter("identificador"))
				...
				|| StringUtils.isEmpty(request
						.getParameter("codigoUnidadOrganica"))) {
			return null;
		} else {
			...
			if (StringUtils.isNotEmpty(request
					.getParameter("serialNumberCertificado"))) {
				retorno.setSerialNumberCertificado(request
						.getParameter("serialNumberCertificado"));
			}
			if (StringUtils.isNotEmpty(request
					.getParameter("numeroProcedimiento"))) {
				retorno.setNumeroProcedimiento(request
						.getParameter("numeroProcedimiento"));
			}

			if (StringUtils.isNotEmpty(request.getParameter("aditional"))) {
				StringTokenizer tmpToken = new StringTokenizer(
						request.getParameter("aditional"), ";;");
			...
			if (StringUtils.isNotEmpty(request
					.getParameter("codigoUnidadOrganica"))
					&& validateOrgano(request
							.getParameter("codigoUnidadOrganica"))) {
				retorno.setCodigoUnidadOrganica(request
						.getParameter("codigoUnidadOrganica"));
			}
	...
	private boolean validateOrgano(String organo) {
		...
		UnidadOrganica unidadOrganica = (UnidadOrganica) eeutilDao.findObjeto(
				UnidadOrganica.class, criterias);
