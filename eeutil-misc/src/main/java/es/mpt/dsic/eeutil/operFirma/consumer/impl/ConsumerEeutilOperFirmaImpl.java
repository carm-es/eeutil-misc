...
public class ConsumerEeutilOperFirmaImpl {
	...
	protected static final Log logger = LogFactory
			.getLog(ConsumerEeutilOperFirmaImpl.class);
	...
	public void configure() throws InSideException {
		if (operFirmaWs == null) {
			...
			try {
				...
				EeUtilServiceImplService ssOperFirma = new EeUtilServiceImplService(
						urlOperFirma);
	...
	public byte[] postProcesarFirma(ApplicationLogin info, byte[] firma)
		...
		configure();
		es.mpt.dsic.eeutil.operFirma.consumer.model.ApplicationLogin credential = new es.mpt.dsic.eeutil.operFirma.consumer.model.ApplicationLogin();
	...
	public ResultadoValidacionInfo validacionFirma(ApplicationLogin info, byte[] firma, String tipoFirma, DatosFirmados datosFirmados)
			throws InSideException {
		configure();
		es.mpt.dsic.eeutil.operFirma.consumer.model.ApplicationLogin credential = new es.mpt.dsic.eeutil.operFirma.consumer.model.ApplicationLogin();
		credential.setIdaplicacion(info.getIdApplicacion());
		credential.setPassword(info.getPassword());
		ResultadoValidacionInfo result = operFirmaWs.validacionFirma(credential, firma, tipoFirma , datosFirmados );
		return result;
	}
