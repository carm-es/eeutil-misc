        ...
        var options = $.extend({
                ...
                onBeforeShow: function(el){
                    ...
                    if(validformat.test(val)){
                        ...
                        var date = new Date()
                        $el.DatePickerSetDate(((date.getMonth()+101)+'').substr(1,2)+'/'+((date.getDate()+100)+'').substr(1,2)+'/'+date.getFullYear(), true);
                    }
                },
                onChange: function(date, el) {
//                    $(el).val(date.getFullYear()+'-'+((date.getMonth()+101)+'').substr(1,2)+'-'+((date.getDate()+100)+'').substr(1,2));
                    $(el).val(((date.getDate()+100)+'').substr(1,2)+'-'+((date.getMonth()+101)+'').substr(1,2)+'-'+date.getFullYear());
                    $(el).DatePickerHide();
                }
            },obj? obj:{}),
            data = $el.data();
