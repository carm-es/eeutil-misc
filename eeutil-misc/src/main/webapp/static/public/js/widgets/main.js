...
$mf.fijar_cabecera = function($cabecera, $contenedor, my_id, my_class, is_table, has_lat){
    ...
    function masScroll_h(){
        $cabecera.one('headmasscroll_h',function(e){
            $(e.currentTarget).wrap('<div id="'+my_id+'h" style="height:'+hh+'px"></div>')
                .addClass(my_class);
            menosScroll_h();
        });
    }
...
$(document).ready(function() {
	...
	$('.multiDropdown, .dropdownbtn-btn').each( function(key,value) {
		$(value).on( 'click', function(e){
				e.preventDefault();
	            $(e.currentTarget).parent().siblings('.dropdownbtn-menu').removeClass('visuallyHidden').addClass('is-open');
