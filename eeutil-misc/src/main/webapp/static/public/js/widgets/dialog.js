$mf.Dialog = function($dialogo, classes, $dialog_parent){
    var self = this,
        ...
        window_w = $('body').width(),
        head_h = $('.mf-header--container').height(),
        cargar_datos = function(callback){
                ...
                if(self.datos.url != undefined){
                    ...
                    $[self.datos.metod || 'post'](self.datos.url, self.datos.data_url).always(function(text){
                        ...
                        if((text.responseText || text).indexOf('<!DOCTYPE html>') == 0){
                            window.history.pushState({"html":text.responseText ||text},"", self.datos.url);
        ...
        max_height = function(max){
            var window_max = !self.datos.drag_container ? ($(window).height()-self.$title_bar.outerHeight()-self.$buttons.outerHeight())*0.8 : $(self.datos.drag_container).outerHeight()*0.8;
    ...
    this.resize_content = function(){
        window_h = $('body').height(),
            window_w = $('body').width(),
            posicionar(self.datos);
    };
    this.close_dialog = function(e){
        if(self.datos.close)eval(self.datos.close)();
    ...
    this.max_dialog = function(e){
        self.$data.show();
        if(self.$buttons.hasClass('has-buttons'))self.$buttons.show();
    ...
    this.$close_button = $('<button class="mf-dialog--btn" title="Cerrar"><span class="mf-icon mf-icon-cancel"></span> Cerrar</button>').appendTo(this.$title_bar)
        .click(self.close_dialog);
    this.$max_button = $('<button class="mf-dialog--btn" title="Maximizar"><span class="mf-icon mf-icon-max-window"></span> Maximizar</button>').appendTo(this.$title_bar)
        .click(self.max_dialog).hide();
    this.$min_button = $('<button class="mf-dialog--btn" title="Minimizar"><span class="mf-icon mf-icon-min-window"></span> Minimizar</button>').appendTo(this.$title_bar)
        .click(self.min_dialog).hide();
...
//    this.$el = $($dialogo);
    ...
    this.appendDialog = function(datos,es_draggable,es_modal){
        ...
        var open_dialog =  function(texto){
            ...
            datos.buttons ? self.$buttons.html(datos.buttons).addClass('has-buttons').show() : self.$buttons.empty().removeClass('has-buttons').hide();
            ...
            if(es_draggable)draggable.call(this);
            if(es_modal)modal.call(this);
            if(datos.visualizer){
            ...
            if(self.datos.visualizer || self.datos.hide_resize_button)self.$min_button.hide();
            if(self.datos.hide_close_button)self.$close_button.hide();
            posicionar(datos);
        }
    ...
    function draggable(){
        this.$title_bar.on('mousedown', function(e){
            var init_x = e.clientX, init_y = e.clientY,
                pos_ini_x = $(e.currentTarget).offset().left,
                pos_ini_y = $(e.currentTarget).offset().top,
                width = $dialogo.outerWidth(),
                height = $dialogo.outerHeight();
            ...
            if(!self.datos.drag_container){
                ...
                $('body').on('mousemove', function(e){
                    var pos = {},
                        $container = $(self.datos.drag_container),
                        pos_container = $container.position(),
                    ...
                    else if(e.clientX-init_x+pos_ini_x > pos_container.left + w_container - width){
                    ...
                    else if(e.clientY-init_y+pos_ini_y > pos_container.top + h_container - height){
                    ...
                    function getSplitData() {
                        if (self.datos.split) {
                            ...
                            if (self.datos.split) {
                                ...
                                self.datos.split.shadowColor = self.datos.split.shadowColor || 'red';
                                self.datos.split.placesToSplit = self.datos.split.placesToSplit || 'top bottom left right';
                                self.datos.split.container = self.datos.split.container || $container;
                                ...
                                if (typeof(self.datos.split.placesToSplit) === 'string') {
                                    self.datos.split.placesToSplit = self.datos.split.placesToSplit.split(' ');
                                }
                            }
                        }
                    ...
                    if (splitData) {
                        function checkSplit(place) {
                            ...
                            return ((place == 'top' && pos.top < self.datos.split.toSplit) ||
                                    ...
                                    (place == 'bottom' && pos.top > $dialogo.innerHeight() - self.datos.split.toSplit) ||
                                    (place == 'right' && pos.top > $dialogo.innerWidth() - self.datos.split.toSplit));

                        }
                        function showShadow (place) {
                            var textPosition = '';
                            if (place == 'top') {
                                textPosition = 'left: 0; top: 0; width: ' + w_container + 'px; height: ' + self.datos.split.toSplit + 'px;';
                            } else if (place == 'bottom') {
                                textPosition = 'left: 0; top: ' + h_container - self.datos.split.toSplit + 'px; width: ' + w_container + 'px; height: ' + self.datos.split.toSplit + 'px;';
                            } else if (place == 'left') {
                                textPosition = 'left: 0; top: 0; width: ' + self.datos.split.toSplit + 'px; height: ' + h_container + 'px;';
                            } else if (place == 'right') {
                                textPosition = 'left: ' + w_container - self.datos.split.toSplit + 'px; top: 0; width: ' + self.datos.split.toSplit + 'px; height: ' + w_container + 'px;';
                            }

                            var shadow = $('<div class="js-dialog_split_shadow" data-place="' + place + '" style="position: absolute; ' +
                                                       'background-color: ' + self.datos.split.shadowColor + ';' +
