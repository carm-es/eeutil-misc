$mf.draw_helps = function(){
    var helps = this,
        mfhelpcontainer = '<div id="mf-help-container" style="display: none;"></div>',
        mfhelpoverlay 	= '<div id="mf-help-overlay" style="display: none;"></div>',
        mfhelpframes 	= '<div id="mf-help-frames" style="display: none;"></div>',
        $mfhelpNavBar 	=   $('<div id="mf-help-navbar">'
            ...
            + '<span>o</span> '
            + '<a href="#!">Necesito saber más, <u>quiero ir a la ayuda</u></a>'
            + '</div>');
    ...
    this.charge_help = function(index, $helpObject){
        ...
        var helpItem = { id: '#mf-help-item-' + index,
            text: null,
            background: null,
        ...
        var offset = $helpObject.offset(),
            $helpItem = $(helpItem.id);
        ...
        if(helpData.mfhelpFrame) {
            ...
            $('#mf-help--item-frame-'+index)
                .width($helpObject.width() + 20)
                .height($helpObject.height() + 20)
                .css({ 'left': offset.left - 10, 'top': offset.top - 10, 'position': 'absolute'});
        };
...
$(document).ready( function () {
    if($('body').hasClass('show_help') && ($.cookie('show-help') != undefined ? eval($.cookie('show-help')) : true)){
        setTimeout(function(){$mf.my_draw_helps = new $mf.draw_helps()},1000);
    }
});
