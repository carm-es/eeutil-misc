...
//        },
        ...
        init_filter = function(key,value){
            var $value = $(value),
                $table = $value.parent().find('table'),
                $filter = $value.children('form');
            if(!$filter.is(':visible')){
                $value.siblings('.grid-header').find('a.mf-filter--title').click(function(){
                    $filter.toggle();
                    $filter.find('.mf-datagrid--filter-content').css('max-height', $table.parent('.mf-datagrid--table-container').outerHeight() - $filter.find('.mf-datagrid--filter-submitbar').outerHeight());
                });
            }
            ...
            $filter.find('.mf-datagrid--filter-content').css('max-height', $table.parent('.mf-datagrid--table-container').outerHeight());
...
//        },
        ...
        pintar = function(data,$el){
            var $data = $(data),
                id = $data.attr('id'),
                $content = $el.closest('mf-datagrid--content'),
