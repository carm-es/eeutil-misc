...
$mf.Formulario = function($el){
    var init = function($el){
        ...
        sup = function(event) {
            ...
            var $ul = $(event.currentTarget).closest('ul'),
                data = $ul.data(),
        ...
        add = function(event) {
            event.preventDefault();
            var $ul =$(event.currentTarget).siblings('ul'),
                data = $ul.data(),
                prototype = data.prototype,
                index = parseInt($ul.attr('data-index')),
                regexp = new RegExp('__' + (data.prototype_name || 'name') + '__',"g"),
    ...
    ($el && $el.find('[type=password]') || $('[type=password]')).one('focus',function(e){
        ...
        $('<button class="mf-showpass--btn">Mostrar contraseña</button>').insertAfter(e.target).on('click', function(e){
            e.preventDefault();
            var $button = $(e.target)
            $el = $button.prev();
    ...
    ($el && ($el.is('.ajax_submit') && $el || $el.find('.ajax_submit')) || $('.ajax_submit')).on('submit', function(e){
        ...
        var $form = $(e.currentTarget),
            $container = ($el && $el.find($form.data('selector_replace')) || $($form.data('selector_replace')));
        $container.trigger('start_refresh');
        $mf.timer.on($container);
        $[$form[0].method]($form[0].action,$form.serializeArray())
            .done(function(data) {
