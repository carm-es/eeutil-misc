$mf.Tabs = function($el){
    var init = function(){
        var $el = $(this),
            $list = $el.find('ul.tabs_group'),
            data = $list.data(),
            $contents = $el.find('.tab_content'),
            pintar = function(e,$li,href) {
            ...
            hover_handle = function(e){
                ...
                if(data.empty && !$li.hasClass('is-selected')){
                    ...
                    if (href && href.indexOf('#') == 0) {
                        var aux = setTimeout(function(){pintar(e,$li,href);}, 200);
        ...
        if($el.hasClass('mf-tabs__hover')){
            $list.on('mouseenter','li a', hover_handle)
                .on('click','li a', click_over_handle);
        }
        else{
