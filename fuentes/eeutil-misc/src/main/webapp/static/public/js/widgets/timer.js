$mf.timer = {
    on: function($el){
        ...
        $veil = $el.find('.mf-veil.timer-veil');
        if($veil.length){
            ...
            $('<div class="mf-veil timer-veil"><p><span></span>Cargando...</p></div>').appendTo($el.css({position:'relative'}))
                .css({position:'absolute'}).data('hash',JSON.stringify([hash]));
        }
        return hash
    },
