$mf.Marquesina = function(event, $el){
    var init = function($obj){
            if($obj.find('.mf-scroll--replace-obj')){
                var $obj_replace = $obj.find('.mf-scroll--replace-obj'),
                    $new_obj = $(template),
                    $list = $new_obj.find('.mf-scroll-list');
            ...
            if(calcular($obj))init_botones($obj);
        },
        init_botones = function($obj){
            var $boton = $obj.find('.js-nav-btn-prev, .js-nav-btn-next'),
                $boton_express = $obj.find('.js-nav-btn-first, .js-nav-btn-last'),
                $boton_centrar = $obj.find('.js-nav-btn-active');
            if(_.isUndefined(event)){
                ...
                $boton_centrar.on('click', function(e) {
                    var min_max = calcular($obj),
                        px = min_max[1]-$obj.find('li.active').position().left+$obj.find('.mf-scroll--wrapper').width()/8;
                    if(px){
                ...
                $boton.on('click', function(e) {
                    var min_max = calcular($obj),
                        wrapper = $obj.find('.mf-scroll--wrapper').width();
        ...
        calcular =function($obj){
            var $data = $obj.find('.mf-scroll-list'),
                $parent = $obj.hasClass('mf-scroll--wrapper') ? $obj : $obj.find('.mf-scroll--wrapper');
            if($parent.width() - $data.width() < 0){
                $obj.addClass('is-scrolling');
                var margen = $parent.width() - $data.width(),
                    position = $data[0].style.left != "" ? $data[0].style.left.substring(0,$data[0].style.left.length-2) : 0,
                    mov_left = margen - (position != "" ? position : 0),
                    mov_right = Math.abs(position) > 0 ? 0 - position : 0;
    ...
    this.refresh = function($obj, $select_opt, prev_px){
        var px = -($select_opt.position().left)+5,
            prev_px = prev_px != undefined ? prev_px : 0;
//        px = min_max[0]> px ? min_max[0] : min_max[1] < px ? min_max[1] : px;
