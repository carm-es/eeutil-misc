$mf.Visualizer = function($el){
    ...
    var ampliar = function(e){
            ...
            $.extend(data, {position_v:'top', position_h:'left'})
        ...
        zoom_in = function(e){
            var $buttons = $(e.currentTarget).parent(),
                $container = $buttons.closest('.mf-visualizer--container, .mf-dialog.mf-visualizer').find('.org-chart-wrapper').children(),
                zoom_min = Number($buttons.attr('data-zoom')),
                zoom_new = $buttons.attr('data-zoom_new');
        ...
        zoom_zero = function(e){
            var $buttons = $(e.currentTarget).parent(),
                $container = $buttons.closest('.mf-visualizer--container, .mf-dialog.mf-visualizer').find('.org-chart-wrapper').children(),
                zoom_new = $buttons.attr('data-zoom');
        ...
        add_buttons = function(a){
            var $el = $(a),
                $buttons = $('<div class="mf-visualizer--toolbar"></div>'),
                data = $el.data();
            init_zoom($buttons,$el.children(".org-chart-wrapper"));
            $('<button class="mf-visualizer--tool mf-visualizer--tool__ampliar"><span class="mf-icon mf-icon-max-window"></span> Ampliar ventana</button>').appendTo($buttons).on('click',ampliar);
            $('<button class="mf-visualizer--tool mf-visualizer--tool__zoom_in"><span class="mf-icon mf-icon-search-plus"></span> Acercar</button>').appendTo($buttons).on('click',zoom_in);
            $('<button class="mf-visualizer--tool mf-visualizer--tool__zoom_out"><span class="mf-icon mf-icon-search-minus"></span> Alejar</button>').appendTo($buttons).on('click',zoom_out);
            $('<button class="mf-visualizer--tool mf-visualizer--tool__zoom_zero"><span class="mf-icon mf-icon-search"></span> 1:1</button>').appendTo($buttons).on('click',zoom_zero);
            data.buttons_ext && $.each(eval(data.buttons_ext),function(i,a){
                $('<button class="mf-visualizer--tool '+a[0]+'">'+a[1]+'</button>').appendTo($buttons).on('click',function(e){
                    eval(a[2])(e)
                });
            });
            return $buttons;
