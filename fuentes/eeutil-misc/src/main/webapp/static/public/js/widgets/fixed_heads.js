$mf.fixed_heads = function($contenedor, my_class, container_max_height, no_resize){
    ...
    if(my_class == undefined)my_class = data && data.clazz != undefined ? data.clazz :'mf-table-fixheads';
    ...
    var table = this,
        $fixheads_main_wrapper = $contenedor.wrap('<div class="'+ my_class +'-wrapper"></div>').parent(),
        resize_headers = function($headertable){
            var change=false,
                $header_cells = $headertable.find('tr').children(),
                resize = function(i,e){
                    $($header_cells[i]).css({  'width': $(e).outerWidth(),
        ...
        resize_height = function(container_max_height){
            ...
            if(table.left_head != undefined)table.left_head.$wrapper.css('height', $contenedor.height());
        },
        mover_cabeceras = function(){
            if(table.top_head != undefined)table.top_head.$el.css('left', -$contenedor.scrollLeft());
            if(table.left_head != undefined)table.left_head.$el.css('top', -$contenedor.scrollTop());
        },
        top_head = function(left,col_left){
            this.$el= table.$el.clone().empty()
                .append(table.$el.children('thead'))
                .addClass(my_class+'--top')
                .css('width',table.$el.width());
            ...
            this.render= function(){
                if(left && typeof col_left != "object"){
                    ...
                    this.$el.find('th:eq('+(col_left != undefined ? col_left : index_th != -1 ? index_th : 0)+')').remove();
                }
                var $top_head = this.$el,
                    $head_table = this.$el.children('thead').clone().prependTo(table.$el);
                if (data.small_cells)$head_table.find('th').empty();
                ...
                this.$el.find('th').each(function(i,a){
                    ...
                    $el.find('.right-border-head').on('mousedown',function(e){
                        $el.on('resize',function(e,width){
                            $head_table.find('th:eq('+i+')').css({'min-width':width,'max-width':width});
                            table.$el.find('tr').each(function(j,a){
                                $(a).find('td:eq('+i+')').css({'min-width':width,'max-width':width});
                            });
                ...
                table.$el.css('top', - table.$el.children('thead').outerHeight())
                    .addClass(my_class+'--main-table');
            }
        },
        left_head = function(col_left){
            var sacar_colum  = function(num_col){
                    var $tabla_aux = $('<table></table>'),
                        $thead_aux = $('<thead></thead>').appendTo($tabla_aux),
                        $tbody_aux = $('<tbody></tbody>').appendTo($tabla_aux);
                    ...
                    if(num_col == undefined){
                        ...
                        $trs.each(function(i,a){
                            $tbody_aux.append(
                                $('<tr></tr>').append(
                                    hasth != -1 ? $(a).children('th:first') : $(a).find('td:first').css('height',$(a).find('td:first').outerHeight())
                                )
                            );
                        });
                    }
                        ...
                        $trs.each(function(i,a){
                            $tbody_aux.append(
                                $('<tr></tr>').append(
                                    $(a).children('td:eq('+num_col+')').css('height',$(a).find('td:first').outerHeight())
                                )
                            );
                        });
        ...
        corner_head = function(){
            this.$el = table.left_head.$el.clone().empty()
                .append(table.left_head.$el.find('th:first'))
                .removeClass(my_class+'--left')
                .addClass(my_class+'--corner')
                .css('height', table.$el.children('thead').outerHeight());
            this.$wrapper = this.$el.wrap('<div class="'+ my_class +'--corner-container"></div>').parent();
            this.render = function(){
                var aux_width = this.$el.outerWidth(),
                    aux_width_children = this.$el.children().outerWidth();
                this.$wrapper.height(table.$el.children('thead').outerHeight()).css({'width': aux_width+2,'overflow':'hidden'});
                $fixheads_main_wrapper.prepend(this.$wrapper);
    ...
    this.render = function(top,left,col_left){
        ...
        if((!data || !data.height ) && !no_resize){
            $(window).resize(function(e){
                setTimeout(function(){
                    var aux_max_height = container_max_height-(window_height-$('body').height());
                    aux_max_height = data && data.min_height != undefined ?
                        (aux_max_height > data.min_height ? aux_max_height : data.min_height):
                        aux_max_height;
